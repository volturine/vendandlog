...
<main class="mx-auto max-w-4xl px-5 pb-20">
	<header class="mb-6 mt-4 flex flex-wrap items-center gap-4">
		...
		<div>
			<h1 class="text-2xl font-bold tracking-tight">
				...
				{#if user.flags_upheld > 0}<span class="vdl-pill vdl-pill-danger align-middle">{user.flags_upheld} flags upheld</span>{/if}
			</h1>
			...
			{#if user.bio}<p class="mt-1 max-w-xl text-[0.85rem] text-[var(--vdl-text-muted)]">{user.bio}</p>{/if}
		</div>
		{#if session.me && session.me.handle !== user.handle}
			<button class="vdl-button vdl-button-secondary ml-auto" onclick={() => (flagOpen = !flagOpen)}>
				Flag user
			</button>
		{/if}
	</header>
	...
	{#if flagOpen}
		<form class="mb-6 space-y-2 rounded-xl border border-[var(--vdl-border)] bg-[var(--vdl-surface)] p-4" onsubmit={submitFlag}>
			<label class="block text-[0.82rem] font-semibold" for="flag-reason">Why are you flagging @{user.handle}? This becomes public.</label>
			<input id="flag-reason" class="vdl-input w-full" bind:value={flagReason} placeholder="No-show twice — chat history is public proof" required />
			<button class="vdl-button vdl-button-primary" disabled={!flagReason.trim()}>Submit flag</button>
		</form>
	{/if}
	...
	<section class="mb-8">
		<h2 class="mb-3 text-[0.68rem] font-bold uppercase tracking-[0.09em] text-[var(--vdl-text-faint)]">Listings — sold ones stay browsable</h2>
	...
	<section>
		<div class="mb-3 flex gap-1 border-b border-[var(--vdl-border-soft)]">
			<button class="rtab" class:on={ratingTab === 'received'} onclick={() => pickRatingTab('received')}>Ratings received</button>
			<button class="rtab" class:on={ratingTab === 'given'} onclick={() => pickRatingTab('given')}>Ratings given</button>
		</div>
		{#if givenLoading}
			<p class="rounded-xl border border-dashed border-[var(--vdl-border)] bg-[var(--vdl-surface)] p-4 text-[0.85rem] text-[var(--vdl-text-muted)]">
				Loading…
			...
			<div class="vdl-card divide-y divide-[var(--vdl-border-soft)]">
				{#each ratings as rating (rating.id)}
					<div class="flex flex-wrap items-baseline gap-x-3 gap-y-1 px-4 py-3 text-[0.86rem]">
						<span class="font-semibold">{rating.rater_handle === user.handle ? rating.ratee_handle : rating.rater_handle}</span>
						...
						<span class="text-[0.72rem] text-[var(--vdl-text-faint)]">
							{timeAgo(rating.created_at)} · <a class="hover:underline" href="/listings/{rating.listing_id}">sale #{rating.listing_id}</a>
						</span>
					</div>
				{/each}
			</div>
