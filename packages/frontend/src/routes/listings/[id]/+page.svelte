<script lang="ts">
	...
	const isParticipant = $derived(
		!!session.me &&
			(!!isSeller || listing.conversations.some((c) => c.buyer_handle === session.me?.handle))
	);
...
<main class="mx-auto max-w-6xl px-4 pb-20 sm:px-5">
	<nav
		class="mb-5 flex flex-wrap items-center gap-2 text-[0.78rem] text-[var(--vdl-text-faint)]"
	...
	<div class="grid gap-5 sm:gap-7 lg:grid-cols-[7fr_5fr]">
		<section class="min-w-0">
			<div class="vdl-card overflow-hidden !rounded-2xl">
				...
				{:else}
					<div
						class="grid aspect-[4/3] w-full place-items-center bg-[var(--vdl-surface-2)] text-[var(--vdl-text-faint)]"
					>
						no photo
					</div>
				{/if}
			...
			<div
				class="mt-6 flex gap-1 overflow-x-auto border-b border-[var(--vdl-border-soft)] [scrollbar-width:none]"
			>
				<button class="tab" class:on={tab === 'description'} onclick={() => (tab = 'description')}
					>Description</button
				>
				<button
					class="tab"
					class:on={tab === 'conversation'}
					onclick={() => (tab = 'conversation')}
				>
					...
					<dl class="mt-5 grid grid-cols-[auto_1fr] gap-x-6 gap-y-1.5 text-[0.84rem]">
						...
						<dd class="font-medium">
							<span
								...
								style="background:{categoryColor(listing.category)}"
							></span>
				...
				<div class="space-y-5">
					...
					{#each listing.conversations as conversation (conversation.id)}
						...
						<div class="vdl-card p-4">
							<div
								class="flex flex-wrap items-center gap-2 border-b border-[var(--vdl-border-soft)] pb-3"
							>
								<strong class="text-[0.9rem]">{conversation.buyer_name}</strong>
								<span class="text-[0.75rem] text-[var(--vdl-text-faint)]"
									>wants to buy · started {timeAgo(conversation.created_at)}</span
								>
								...
								{#if !conversation.is_public && (isSeller || session.me?.handle === conversation.buyer_handle)}
									<button
										class="vdl-button vdl-button-quiet ml-auto !px-2 !py-1 text-[0.75rem]"
										onclick={() => unhide(conversation.id)}
									>
										Make public now
									</button>
								{/if}
							...
							{#if !conversation.is_public && listing.status !== 'sold' && (isSeller || session.me?.handle === conversation.buyer_handle)}
								...
								>
									<input
										class="vdl-input flex-1"
										placeholder="Write a message…"
										bind:value={messageDraft[conversation.id]}
									/>
									<button class="vdl-button vdl-button-primary" disabled={busy}>Send</button>
								</form>
							{:else if conversation.is_public && listing.status === 'sold'}
								<p class="mt-2 text-[0.75rem] text-[var(--vdl-text-faint)]">
									This conversation is part of the permanent public record.
								</p>
							{/if}
						</div>
					{/each}
					...
					{#if session.me && !isSeller && listing.status === 'active' && !listing.conversations.some((c) => c.buyer_handle === session.me?.handle)}
						<button class="vdl-button vdl-button-secondary w-full" onclick={startConversation}
						...
						{#each listing.events as event (event.id)}
							<div class="log-item {eventTone[event.kind] ?? ''}">
								...
								<p class="mt-1 flex items-center gap-2 text-[0.72rem] text-[var(--vdl-text-faint)]">
									...
									<span
										class="rounded bg-[var(--vdl-surface)] px-1.5 py-0.5 font-mono text-[0.68rem]"
										>#{event.hash}</span
									>
								</p>
							</div>
		...
		<aside class="min-w-0 space-y-4">
			<div class="vdl-card p-5">
				<div class="flex items-baseline gap-2">
					<span class="text-[1.7rem] font-bold tracking-tight"
						>{formatPrice(listing.price_cents)}</span
					>
					{#if listing.previous_price_cents && listing.previous_price_cents > listing.price_cents}
						<span class="text-[0.95rem] text-[var(--vdl-text-faint)] line-through"
							>{formatPrice(listing.previous_price_cents)}</span
						>
					{/if}
				</div>
				{#if dropPct}
				...
				{#if isSeller && listing.status === 'active'}
					<div class="mt-4 flex gap-2">
						<button class="vdl-button vdl-button-primary flex-1" disabled={busy} onclick={markSold}
							>Mark as sold</button
						>
						<button class="vdl-button vdl-button-secondary" disabled={busy} onclick={withdraw}
							>Withdraw</button
						>
					</div>
				{:else if listing.status === 'sold'}
					<p
						class="mt-4 rounded-lg bg-[var(--vdl-success-subtle)] p-3 text-[0.8rem] font-medium text-[var(--vdl-success)]"
					>
					...
					{#if session.me && isParticipant && !listing.my_rating}
						<form
							class="mt-4 space-y-2.5 rounded-xl border border-[var(--vdl-border-soft)] bg-[var(--vdl-surface)] p-3.5"
							onsubmit={submitRating}
						>
							<p class="text-[0.82rem] font-semibold">
								Rate {isSeller
									? (listing.conversations[0]?.buyer_name ?? 'your buyer')
									: listing.seller.name}
							</p>
							<div class="flex gap-1">
								{#each [1, 2, 3, 4, 5] as star (star)}
									<button
										type="button"
										class="text-xl {star <= ratingStars
											? 'text-amber-500'
											: 'text-[var(--vdl-border)]'}"
							...
							<textarea
								class="vdl-input w-full min-h-16"
								bind:value={ratingText}
								placeholder="How did it go? This becomes public."></textarea>
							<button class="vdl-button vdl-button-primary w-full" disabled={busy}
								>Submit rating</button
							>
						</form>
					{:else if listing.my_rating}
						<p class="mt-3 text-[0.78rem] text-[var(--vdl-text-muted)]">
							You rated this counterparty {listing.my_rating.stars}★ —
							<a
								class="text-[var(--vdl-accent)] underline"
								href="/users/{listing.my_rating.ratee_handle}">see their profile</a
							>.
						</p>
					{/if}
				{:else if listing.status === 'withdrawn'}
					<p
						class="mt-4 rounded-lg bg-[var(--vdl-warning-subtle)] p-3 text-[0.8rem] font-medium text-[var(--vdl-warning)]"
					>
						Withdrawn — this stays public per the record.
					</p>
				{:else if session.me && !isSeller}
					<button
						class="vdl-button vdl-button-primary mt-4 w-full"
						disabled={busy}
				...
				<div
					class="mt-4 rounded-xl border border-[var(--vdl-border-soft)] bg-[var(--vdl-surface)] p-3.5"
				>
					...
					<a
						href="/users/{listing.seller.handle}"
						class="mt-2 inline-block text-[0.8rem] font-medium text-[var(--vdl-accent)] hover:underline"
					>
						View profile →
					</a>
				</div>
				...
				<div class="vdl-card p-4">
					...
					{#each listing.similar as similar (similar.id)}
						<a
							...
							class="flex items-center gap-3 rounded-lg p-1.5 transition-colors hover:bg-[var(--vdl-hover)]"
						>
	...
	{#if notice}
		...
		>
			{notice}
			<button class="ml-3 font-bold opacity-70 hover:opacity-100" onclick={() => (notice = null)}
				>✕</button
			>
		</div>
	{/if}
